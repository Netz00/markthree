...
class Doc extends React.Component {
  ...
  initializeFromDocList(docList, caretAt) {
    if (docList.filter((d) => !d).length === 0) {
      ...
      Array.from(document.querySelector('#m2-doc').children).forEach(
        (el, i) => {
  ...
  sync(lines, forceSync) {
    ...
    console.log(
      `is signed in: ${this.props.gapi.auth2
        .getAuthInstance()
        .isSignedIn.get()}`
    )
    ...
    for (i = 0; i < docMetadata.pageIds.length; i++) {
      ...
      ).map((id) => ({ id, text: this.state.doc[id] }))
    ...
    let newLines = _.slice(lines, startIndex, endIndex).map((id) => ({
      id,
    ...
    let caretAt =
      $(sel.anchorNode).closest('#m2-doc > *').attr('id') || docMetadata.caretAt
    // cache all pageIds
    ...
    const pagesToAdd = _.difference(pageIds, docMetadata.pageIds).map(
      (pageId) => ({ name: pageId, data: pages[pageId] })
    )
    ...
    return this.syncUtils
      ...
      .then((results) => {
        ...
        docMetadata.pageLengths = docMetadata.pageIds.map(
          (pageId) => pages[pageId].length
        )
      ...
      .then((keys) => {
        ...
        const localPages = keys.filter((k) =>
          k.startsWith(`${this.props.currentDoc}.`)
        )
        const removeThese = _.difference(
          localPages,
  ...
  enterEditMode() {
    ...
    const originalAnchorText =
      sel.anchorNode && sel.anchorNode.data
        ? sel.anchorNode.data.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&')
        : 0
    ...
    if (sel.anchorNode && selectedBlock && selectedBlock[0]) {
      ...
      if (selectedBlock.attr('id')) {
        ...
        renderedMarkdown =
          this.turndownService.turndown(selectedBlock[0].outerHTML) || '\u200B'
      }
      selectedBlock.text(renderedMarkdown)
      var range = document.createRange()
      let offset
      if (selectedBlock[0].firstChild && selectedBlock[0].firstChild.data) {
        const stringMatch = selectedBlock[0].firstChild.data.match(
          new RegExp(originalAnchorText)
        )
      ...
      range.setStart(
        selectedBlock[0].firstChild,
        Math.min(offset, selectedBlock[0].firstChild.length)
      )
      range.collapse(true)
      sel.removeAllRanges()
      sel.addRange(range)
  ...
  handleScroll() {
    ...
    if (endIndex - startIndex > 500) {
      ...
      if (scrollPercent < 0.2) {
        document
          .querySelectorAll(`#m2-doc>*:nth-child(n+250)`)
          .forEach((el) => {
            el.remove()
          })
      ...
      if (scrollPercent > 0.8) {
        document
          .querySelectorAll(
            `#m2-doc > *:nth-child(-n+${
              document.querySelectorAll('#m2-doc > *').length - 250
            })`
          )
          .forEach((el) => {
            el.remove()
  ...
  getNodeForBlock(block) {
    ...
    const mentionOrHashtagRegex = new RegExp(
      '(?:^([#@][^\\s#@]+))|(?:[\\s\u200B]([#@][^\\s#@]+))',
      'g'
    )
    ...
    checkboxes.each((i, el) => {
      el.replaceWith(
        $(`<span contenteditable="false" idx="${i}">${el.outerHTML}</span>`)[0]
      )
    })
    ...
    if (block.startsWith('// ')) {
      renderedNode = $(
        `<div class="m2-bookmark">${block.replace('// ', '')}<hr /></div>`
      )
    }
  ...
  initializeEditor() {
    ...
    document
      .querySelector('#m2-doc')
      .addEventListener('paste', () => setTimeout(this.getAllLines, 50))
      ...
      if (e.inputType === 'deleteContentBackward') {
        if (!document.querySelector('#m2-doc > *')) {
          ...
          if (selectedBlock && selectedBlock[0]) {
            const doc = _.clone(this.state.doc)
            doc[selectedBlock[0].id] = this.turndownService.turndown(
              selectedBlock[0].outerHTML
            )
            this.setState({ doc })
          }
        }
    ...
    modal.querySelector('input').addEventListener('change', function (e) {
      ...
      if (e.target.files && e.target.files[0]) {
        ...
        reader.onload = function (e) {
          ...
          that.syncUtils
            .createImage(`m2img.${fileName}`, e.target.result)
            .then((result) => {
              console.log(result)
              $(modal.querySelector('input')).data(
                'webUrl',
    ...
    $('#m2-doc').on('keydown keyup mouseup', (e) => {
      ...
      const s =
        sel.anchorNode &&
        sel.anchorNode.data &&
        sel.anchorNode.data.substring(sel.anchorOffset - 50, sel.anchorOffset)
      const autocompleteRegex = new RegExp(
        '(?:^([#@:/][^\\s#@]*$))|(?:[\\s\u200B]([#@:/][^\\s#@]*$))'
      )
      ...
      if (autocompleteRegex.test(s)) {
          ...
          results = _.uniq(
            _.flatten(
              this.state.allLines
                .map(
                  (id) =>
                    id !== selectedBlock[0].id &&
                    this.state.doc[id].match(findRegex)
                )
                .filter((r) => r)
            )
          )
        ...
        $('#m2-autocomplete').html(
          results.map((r) => `<div>${r.trim()}</div>`).join('\n')
        )
        autocompleteSelectedIndex &&
          $(
            `#m2-autocomplete div:nth-child(${autocompleteSelectedIndex})`
          ...
          if (autocompleteActive) {
            ...
            if (rects.length > 0) {
              ...
              autocompleteDropdownAbove =
                rects[0].top / window.outerHeight < 0.6
        ...
        if (!autocompleteActive) {
          ...
          if (rects.length > 0) {
            $('#m2-autocomplete').css(
              'left',
              rects[0].left - 10 * matchedText.length
            )
          }
        }
        ...
        function selectEntry() {
          ...
          const endOfWord = sel.anchorNode.data
            .substring(sel.anchorOffset)
            .match(/[^\s]*/)
          const endOffset = endOfWord
            ? caretOffset + endOfWord[0].length
            : caretOffset
          ...
          if (
            newText.split(' ')[1] &&
            emoji.getUnicode(newText.split(' ')[1])
          ) {
        ...
        if (e.key === 'ArrowDown' && e.type === 'keydown' && results.length) {
          ...
          if (!autocompleteDropdownAbove) {
            ...
            // wrap around
            autocompleteSelectedIndex =
              autocompleteSelectedIndex > results.length
                ? 1
                : autocompleteSelectedIndex
            e.preventDefault()
          }
          ...
          if (
            ...
            if (sel.getRangeAt && sel.rangeCount) {
              ...
              sel.anchorNode.nextSibling &&
                sel.collapse(
                  sel.anchorNode.nextSibling,
                  sel.anchorNode.nextSibling.length
                )
            }
          } else {
            ...
            const contentWithTextRemoved = doc[selectedBlock[0].id].replace(
              initialContent,
              ''
            )
      ...
      // save markdown
      if (
        this.oldSelectedBlock &&
        this.oldSelectedBlock[0] &&
        selectedBlock &&
        selectedBlock[0]
      ) {
        ...
        if (
          !id ||
          !this.oldSelectedBlock[0].isSameNode(document.getElementById(id))
        ) {
          id = shortid.generate()
        ...
        if (!this.oldSelectedBlock[0].isSameNode(selectedBlock[0])) {
          const blocks =
            this.oldSelectedBlock[0].nodeName === 'PRE'
              ? [markdown]
              : markdown.split('\n\n')
    ...
    $('#m2-doc').on('focusout', (e) => {
      ...
      if (oldSelectedBlock[0]) {
        ...
        if (
          !id ||
          !oldSelectedBlock[0].isSameNode(document.getElementById(id))
        ) {
          id = shortid.generate()
        ...
        const blocks =
          oldSelectedBlock[0].nodeName === 'PRE'
            ? [markdown]
            : markdown.split('\n\n')
        const nodes = blocks.map((block, i) => {
          block = block.replace(/\/now/gi, moment().format('LLL'))
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (this.props.goToBlock !== nextProps.goToBlock) {
      const docList = this.state.allLines.map((id) => ({
        id,
        text: this.state.doc[id],
      }))
    ...
    this.syncUtils = this.props.offlineMode
      ? syncUtilsOffline()
      : syncUtils(this.props.gapi)
    this.props.offlineMode && !this.props.tryItNow
      ? $('.m2-offline').show()
      : $('.m2-offline').hide()
  ...
  componentDidMount() {
    ...
    if (!this.props.tryItNow) {
      ...
      this.syncUtils
        .initializeData(this.props.currentDoc, docMetadataDefault)
        .then((docMetadata) => {
          if (this._isMounted) {
            ...
            this.getDocList(docMetadata).then((docList) => {
              if (this._isMounted) {
                ...
                this.initializeFromDocList(
                  docList,
                  docMetadata.caretAt || docList[0].id
                )
                this.showReminders()
              }
            })
  ...
  bindCheckboxEvents() {
    ...
    $(`input[type=checkbox]`).change(function () {
      this.checked
        ? $(this).closest('li').addClass('m2-todo-done')
        : $(this).closest('li').removeClass('m2-todo-done')
      ...
      that.state.doc[id].split('\n').forEach((line) => {
        if (/(?:[-*+]|(?:[0-9]+\.))\s+\[[x\s]\]/.test(line)) {
          if (idx === parseInt(this.parentElement.getAttribute('idx'))) {
            if (this.checked) {
              line = line.replace(/(?:[-*+]|(?:[0-9]+\.))\s+\[\s\]/, (match) =>
                match.replace('[ ]', '[x]')
              )
            } else {
              line = line.replace(/(?:[-*+]|(?:[0-9]+\.))\s+\[x\]/, (match) =>
                match.replace('[x]', '[ ]')
              )
            }
          }
    ...
    return (
      <div>
        ...
        >
          <div>
            You have a reminder.{' '}
            <a href="#" onClick={this.viewReminder}>
              View
            </a>
          </div>
